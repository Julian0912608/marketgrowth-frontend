...
import {
  Sparkles, Instagram, Copy, RefreshCw, CheckCircle,
  Zap, TrendingUp, ShoppingBag, Megaphone, Image,
  Layers, Video, Download, ChevronRight, ChevronLeft,
  Loader2, AlertCircle,
} from 'lucide-react';
...
function PostCard({ post, index, format }: { post: GeneratedPost; index: number; format: ContentFormat }) {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      if (res.data.creative?.imageUrl) {
        setImageUrl(res.data.creative.imageUrl);
      }
    } catch {}
  ...
  const downloadImage = () => {
    ...
    a.href = imageUrl;
    a.download = `marketgrow-creative-${index + 1}.jpg`;
    a.click();
  };
    ...
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
      ...
      {imageUrl ? (
        <div className="relative">
          <img src={imageUrl} alt="Generated creative" className="w-full aspect-square object-cover" />
          <button
        ...
        <div className="aspect-square bg-slate-900/50 border-b border-slate-700/50 flex flex-col items-center justify-center gap-3 text-slate-500">
          {imageLoading ? (
            ...
            <>
              ...
              <span className="text-xs px-4 text-center opacity-60">
      ...
      <div className="p-5">
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                    Slide {slideIndex + 1} of {post.slides.length}
                  </span>
                ...
                <div className="flex items-center justify-center gap-1 mt-3">
                  {post.slides.map((_, i) => (
                    <button key={i} onClick={() => setSlideIndex(i)}
                      className={`w-1.5 h-1.5 rounded-full transition-all ${i === slideIndex ? 'bg-brand-400 w-3' : 'bg-slate-600'}`} />
                  ))}
                </div>
              </div>
            )}
            <div className="mb-3">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Caption</span>
              <p className="text-sm text-slate-300 mt-1">{post.caption}</p>
            </div>
            <p className="text-sm text-white mb-4">{post.cta}</p>
          </>
        ) : (
          <>
        ...
        {post.hashtags && post.hashtags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {post.hashtags.map((h, i) => (
              <span key={i} className="text-xs text-brand-400 bg-brand-600/10 px-2 py-0.5 rounded-full">
                #{h}
              </span>
            ))}
          </div>
        ...
        {post.image_prompt && !imageUrl && (
          <div className="mb-4 p-3 bg-slate-900/50 rounded-xl border border-slate-700/30">
            ...
            <p className="text-xs text-slate-500 leading-relaxed line-clamp-3">{post.image_prompt}</p>
          </div>
        )}
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-[360px_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* Count + image generation + Generate */}
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-xs text-slate-500">Nano Banana visuals (uses AI credits)</p>
              </div>
              <button
                onClick={() => setGenerateImages(!generateImages)}
                className={`w-10 h-5 rounded-full transition-all relative ${generateImages ? 'bg-brand-600' : 'bg-slate-700'}`}
              >
                <div className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-all ${generateImages ? 'left-5.5' : 'left-0.5'}`}
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center h-80 gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-400 text-sm">Analysing your store data and creating posts</p>
              </div>
            </div>
          ) : posts.length === 0 ? (
            ...
            <div className={`grid gap-6 ${posts.length === 1 ? 'max-w-sm' : 'sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3'}`}>
              {posts.map((post, i) => (
