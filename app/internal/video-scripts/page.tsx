...
const card  = 'bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 mb-4';
const label = 'text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 block';
const btn   = (active: boolean) => `w-full text-left px-3 py-2.5 rounded-xl text-sm transition-all mb-2 ${active ? 'bg-brand-600/20 border border-brand-500/40 text-brand-300' : 'bg-slate-700/30 text-slate-400 hover:text-white'}`;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center text-lg">🎬</div>
        <h1 className="font-display text-2xl font-800 text-white">Video Script Generator</h1>
        <span className="text-xs bg-purple-900/50 text-purple-400 px-2.5 py-1 rounded-full font-semibold">INTERNAL</span>
      </div>
      <p className="text-slate-500 text-sm mb-8 ml-12">Generate AI voiceover scripts for MarketGrow social content</p>
      ...
      <div className="grid lg:grid-cols-[360px_1fr] gap-6">
        ...
        <div>
          <div className={card}>
            <span className={label}>Data Scenario</span>
            {SCENARIOS.map(s => (
              <button key={s.id} onClick={() => setScenarioId(s.id)} className={btn(scenarioId === s.id)}>
            ...
            <div className="mt-3 bg-slate-900/60 rounded-xl p-3">
              ...
              {[['Revenue', scenario.revenue], ['Ad spend', scenario.adSpend], ['Real ROAS', scenario.realRoas], ['Campaigns', String(scenario.campaigns)], ['Margin', scenario.margin]].map(([k,v]) => (
              ...
              <div className="mt-2 pt-2 border-t border-slate-700 text-xs text-amber-400">💡 {scenario.insight}</div>
            </div>
          </div>
          ...
          <div className={card}>
            ...
            {FORMATS.map(f => (
              <button key={f.id} onClick={() => setFormatId(f.id)} className={btn(formatId === f.id)}>
          ...
          <div className={card}>
            ...
            {ANGLES.map(a => (
              <button key={a.id} onClick={() => setAngleId(a.id)} className={btn(angleId === a.id)}>
          ...
          <div className={card}>
            <span className={label}>Number of scripts</span>
            <div className="flex gap-2 mb-4">
              {[1, 3].map(n => (
                <button key={n} onClick={() => setCount(n)} className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${count === n ? 'bg-brand-600 text-white' : 'bg-slate-700 text-slate-400 hover:text-white'}`}>{n}</button>
              ))}
            </div>
            <button onClick={generate} disabled={loading} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 disabled:opacity-50 text-white font-bold py-3 rounded-xl text-sm transition-all">
              {loading ? <><RefreshCw className="w-4 h-4 animate-spin" /> Generating...</> : <><Sparkles className="w-4 h-4" /> Generate scripts</>}
            </button>
        ...
        <div>
          ...
          {loading && (
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 animate-pulse">
              {[100,75,55,100,65].map((w,i) => <div key={i} className="h-3 bg-slate-700 rounded mb-3" style={{width:`${w}%`}} />)}
            </div>
          )}

          {results.map((r, i) => (
            <div key={i} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 mb-4">
              {results.length > 1 && <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Script {i+1} of {results.length}</div>}

              {/* Hook */}
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">⚡ Hook — first 2 seconds</span>
                  <button onClick={() => copy(r.hook, `hook-${i}`)} className={`text-xs px-2.5 py-1 rounded-lg flex items-center gap-1 ${copied===`hook-${i}` ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400 hover:text-white'}`}>
                    {copied===`hook-${i}` ? <><CheckCircle className="w-3 h-3"/>Copied</> : <><Copy className="w-3 h-3"/>Copy</>}
                  </button>
                </div>
              ...
              <div className="bg-slate-900/60 rounded-xl p-4 mb-4">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">🎙️ Full Script → paste into ElevenLabs</span>
                  <button onClick={() => copy(r.script, `script-${i}`)} className={`text-xs px-2.5 py-1 rounded-lg flex items-center gap-1 ${copied===`script-${i}` ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400 hover:text-white'}`}>
                    {copied===`script-${i}` ? <><CheckCircle className="w-3 h-3"/>Copied</> : <><Copy className="w-3 h-3"/>Copy</>}
                  </button>
                </div>
                <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap" style={{fontFamily:'Georgia,serif'}}>{r.script}</p
